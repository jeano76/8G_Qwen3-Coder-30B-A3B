...
**Result: a 35B-parameter model (Qwen3.6-35B-A3B, MTP build) with a 64K context on a GPU that holds neither** — **44.4 tok/s generation and 923 t/s prompt processing, held steady across repeated requests** at a real 22K-token Cline working context, up from 21.1 tok/s on the model this document started with. This is the reconfirmed session-shape number, not a single benchmark run — see the correction below. The last ~17% of that generation figure comes from MTP, measured in [Speculative decoding](#speculative-decoding-mtp-pays-ngram-mod-is-unproven). `ngram-mod` was stacked on top of it through several revisions of this document — every benchmark here said it was free — until a session sending repeated requests against one live server found it regresses generation **17%** from the second request on. It has been removed from production; see [the correction](#correction-2026-08-28-ngram-mod-regresses-across-repeated-requests).
